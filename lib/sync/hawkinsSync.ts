...
export async function runHawkinsSync(
  ...
  try {
    ...
    const athletesRes = await fetch(`${apiBase}/athletes`, {
      headers: authHeaders,
    });
    if (!athletesRes.ok) {
    ...
    const athleteList = Array.isArray(hawkinsAthletes)
      ? hawkinsAthletes
      : Array.isArray((hawkinsAthletes as { data?: unknown })?.data)
        ? (hawkinsAthletes as { data: unknown[] }).data
        : [];
