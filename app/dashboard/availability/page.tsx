...
/**
 * Clinician availability manager (Cliniko-style).
 * - Recurring weekly hours -> `availability` (per weekday, multiple windows ok).
 * - One-off changes -> `availability_exceptions` (block a day/part-day, or add
 *   hours on a normally-closed day).
 * The public booking engine already reads both, so edits here immediately change
 * what patients can book. Staff RLS lets us write these directly from the client.
 */
...
export default function AvailabilityPage() {
  ...
  function editWindow(wd: number, key: string, field: "start" | "end", value: string) {
    setWeek((prev) => ({
      ...prev,
      [wd]: (prev[wd] ?? []).map((w) => (w.key === key ? { ...w, [field]: value } : w)),
    }));
  }
  ...
  async function saveWeek() {
    ...
    // Replace this clinician's recurring hours with the current set.
    const del = await supabase.from("availability").delete().eq("clinician_id", profileId);
    if (del.error) {
    ...
    <main className="min-h-screen bg-[#f8fafc] text-slate-900 athlete-frosted" data-theme="light">
      ...
      <section className="mx-auto max-w-3xl px-4 pt-8 pb-20">
        ...
        <p className="mt-1 text-sm text-slate-400">
          Set your recurring hours and one-off changes. These control what patients can book online.
        </p>

        {loading ? <p className="mt-6 text-xs text-slate-500">Loading…</p> : null}

        {/* Weekly hours */}
        <div className="mt-6 rounded-xl border border-slate-800 bg-slate-900/50 p-5">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-semibold text-slate-100">Weekly hours</h2>
            <button
              type="button"
                    ...
                    (week[wd] ?? []).map((w) => (
                      <div key={w.key} className="flex items-center gap-2">
                        <input
                          type="time"
                          value={w.start}
                          onChange={(e) => editWindow(wd, w.key, "start", e.target.value)}
                        ...
                        <input
                          type="time"
                          value={w.end}
                          onChange={(e) => editWindow(wd, w.key, "end", e.target.value)}
                          className="rounded-lg border border-slate-700 bg-slate-950 px-2 py-1 text-sm text-slate-200"
                        />
                        <button
                          type="button"
                          onClick={() => removeWindow(wd, w.key)}
        ...
        <div className="mt-6 rounded-xl border border-slate-800 bg-slate-900/50 p-5">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <select
                value={exMode}
                onChange={(e) => setExMode(e.target.value as "block" | "open")}
                className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-200"
              >
          ...
          <label className="mt-3 flex items-center gap-2 text-sm text-slate-300">
            <input
              type="checkbox"
              checked={exAllDay}
              onChange={(e) => setExAllDay(e.target.checked)}
              disabled={exMode === "open"}
              className="h-4 w-4"
            />
            All day
          </label>
          {exMode === "open" && exAllDay ? (
            <p className="mt-1 text-xs text-amber-300">Adding hours needs a start and end time.</p>
          ) : null}
          ...
          <button
            ...
            disabled={exSaving || (exMode === "open" && exAllDay)}
            className="mt-4 rounded-lg border border-slate-600 bg-slate-800 px-4 py-2 text-xs font-medium text-slate-100 hover:bg-slate-700 disabled:opacity-50"
          >
            {exSaving ? "Adding…" : "Add"}
          </button>
                    ...
                    <span className="text-slate-200">
                      <span
                        className={`mr-2 inline-block rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase ${
                          ex.is_available ? "bg-lime-500/20 text-lime-200" : "bg-rose-500/20 text-rose-200"
                        }`}
                      >
